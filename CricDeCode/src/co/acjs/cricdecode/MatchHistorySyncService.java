...
public class MatchHistorySyncService extends IntentService{
	...
	protected void onHandleIntent(Intent intent){
		if(AccessSharedPrefs.mPrefs.getString("MatchHistorySyncServiceCalled", CDCAppClass.DOESNT_NEED_TO_BE_CALLED).equals(CDCAppClass.NEEDS_TO_BE_CALLED)){
			...
			ServerDBSubInfiSync.query(ServerDBSubInfiSync.class, new StackMobQuery().field(new StackMobQueryField("user_id").isEqualTo(AccessSharedPrefs.mPrefs.getString("id", ""))).field(new StackMobQueryField("validuntil_ts_msec").isGreaterThan(tm)), new StackMobQueryCallback<ServerDBSubInfiSync>(){
				@Override
