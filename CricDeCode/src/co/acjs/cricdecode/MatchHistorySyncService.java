...
public class MatchHistorySyncService extends IntentService {
	...
	public void sendData(String regids) {
		...
		final Cursor t = getContentResolver().query(
				...
				new String[] { PerformanceDb.KEY_ROWID,
						...
						PerformanceDb.KEY_INNING, PerformanceDb.KEY_MATCHID,
		...
		Log.w("MatchHistorySync",
				"Number of Un Synced Performances " + t.getCount());
		if (c.getCount() != 0) {
			c.moveToFirst();
			do {
				ServerDBCricketMatch cm = new ServerDBCricketMatch(
						...
						Integer.parseInt(AccessSharedPrefs.mPrefs.getString(
								"device_id", "")),
						c.getString(c
						...
						c.getString(c.getColumnIndexOrThrow(MatchDb.KEY_VENUE)),
						Integer.parseInt(c.getString(c
						...
						c.getString(c.getColumnIndexOrThrow(MatchDb.KEY_LEVEL)),
						c.getString(c
						...
						0);
				cm.setID(AccessSharedPrefs.mPrefs.getString("device_id", "")
						+ "A"
						+ c.getString(c
								.getColumnIndexOrThrow(MatchDb.KEY_ROWID))
						+ "B" + AccessSharedPrefs.mPrefs.getString("id", ""));
					...
					public void success(String arg0) {
						final Cursor c1 = getContentResolver()
								.query(CricDeCodeContentProvider.CONTENT_URI_PERFORMANCE,
										...
										null);
						Log.w("MatchHistorySync",
								"Current Performances " + c1.getCount());
						...
						if (c1.getCount() != 0) {
							...
							do {
								...
								sp.setID(AccessSharedPrefs.mPrefs.getString(
										"device_id", "")
										+ "A"
										+ c1.getString(c1
												.getColumnIndexOrThrow(PerformanceDb.KEY_MATCHID))
													...
													.query(ServerDBAndroidDevices.class,
															...
															new StackMobQueryCallback<ServerDBAndroidDevices>() {
																...
																public void success(
																	...
																	final Cursor c2 = getContentResolver()
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (AccessSharedPrefs.mPrefs.getString("MatchHistorySyncServiceCalled",
			...
			if (AccessSharedPrefs.mPrefs.getString("infi_sync", "no").equals(
				...
				while (jsonParser.isOnline(who)) {
					...
					jn = jsonParser.makeHttpRequest(who.getResources()
							.getString(R.string.purchase_infi_sync), "POST",
							params, who);
			...
			/*
			 * 
			 * if(AccessSharedPrefs.mPrefs.getString("infi_sync",
			 * "no").equals("yes")){
			 * ServerDBSubInfiSync.query(ServerDBSubInfiSync.class, new
			 ...
			 * long now = new Date().getTime(); if((arg0.size() > 0)){ long t1 =
			 * arg0.get(0).getValidUntilTs(); int m = 0; for(int i = 1; i <
			 * arg0.size(); i++){ if(t1 < arg0.get(i).getValidUntilTs()){ t1 =
			 * arg0.get(i).getValidUntilTs(); m = i; } } final
			 * ServerDBSubInfiSync max = arg0.get(m); if(now <
			 * arg0.get(m).getValidUntilTs()){ AccessSharedPrefs.setString(who,
			 * "infi_sync", "yes"); sendData(); }else{
			 ...
			 * = regids + " " + arg0.get(i).getGcmId(); } List<NameValuePair>
			 * params = new ArrayList<NameValuePair>(); params.add(new
			 * BasicNameValuePair("SendToArrays", regids)); params.add(new
			 * BasicNameValuePair("product_id", "sub_infi_sync")); JSONObject jo
			 * = new JSONObject(); try{ jo.put("orderId", max.getOrderId());
			 * jo.put("Token", max.getToken()); jo.put("Sign", max.getSign());
			 * }catch(JSONException e){} params.add(new
			 * BasicNameValuePair("json", jo.toString())); params.add(new
			 * BasicNameValuePair("id", AccessSharedPrefs.mPrefs.getString("id",
			 * "")));
			 * 
			 * final JSONParser jsonParser = new JSONParser(); int trial = 1;
			 * JSONObject jn = null; while(jsonParser.isOnline(who)){
			 * Log.w("JSONParser", "SubinfiSync:: Called"); jn =
			 * jsonParser.makeHttpRequest
			 * (who.getResources().getString(R.string.purchase_infi_sync),
			 * "POST", params, who); Log.w("JSON returned", "SubinfiSync:: " +
			 * jn); Log.w("trial value", "SubinfiSync:: " + trial); if(jn !=
			 * null) break; try{ Thread.sleep(10 * trial);
			 * }catch(InterruptedException e){} trial++; if(trial == 50) break;
			 * } try{ List<NameValuePair> params1 = new
			 * ArrayList<NameValuePair>(); String body1 = "User Id:" +
			 * AccessSharedPrefs.mPrefs.getString("id", "") + "<br /> JSON:" +
			 * jo.toString() + "<br />reply: " + jn.toString() + "<br />trial: "
			 * + trial; params1.add(new BasicNameValuePair("body1", body1));
			 * if(jn.getInt("status") == 1){ params1.add(new
			 * BasicNameValuePair("subj",
			 * AccessSharedPrefs.mPrefs.getString("f_name", "") + " " +
			 * AccessSharedPrefs.mPrefs.getString("l_name", "") +
			 * " MatchHistSync-InfiSync success")); }else{ params1.add(new
			 * BasicNameValuePair("subj",
			 * AccessSharedPrefs.mPrefs.getString("f_name", "") + " " +
			 * AccessSharedPrefs.mPrefs.getString("l_name", "") +
			 * " MatchHistSync-InfiSync failure")); }
			 * jsonParser.makeHttpRequest(
			 * getResources().getString(R.string.send_mail), "POST", params1,
			 * who); }catch(Exception e){} try{ if(jn.getInt("status") == 1){
			 * AccessSharedPrefs.setString(who, "infi_sync", "yes"); sendData();
			 * }else if(jn.getInt("status") == 0){
			 * AccessSharedPrefs.setString(who, "infi_sync", "no");
			 ...
			 * AccessSharedPrefs.setString(who, "infi_sync", "no"); } } });
			 * }else if(AccessSharedPrefs.mPrefs.getString("sync",
			 * "no").equals("yes")){
			 * ServerDBSubSync.query(ServerDBSubSync.class, new
			 * StackMobQuery().field(new
			 * StackMobQueryField("user_id").isEqualTo(
			 * AccessSharedPrefs.mPrefs.getString("id", ""))), new
			 ...
			 * @Override public void failure(StackMobException arg0){}
			 * 
			 * @Override public void success(List<ServerDBSubSync> arg0){ long
			 * now = new Date().getTime(); if((arg0.size() > 0)){ long t1 =
			 * arg0.get(0).getValidUntilTs(); int m = 0; for(int i = 1; i <
			 * arg0.size(); i++){ if(t1 < arg0.get(i).getValidUntilTs()){ t1 =
			 * arg0.get(i).getValidUntilTs(); m = i; } } final ServerDBSubSync
			 * max = arg0.get(m); if(now < arg0.get(m).getValidUntilTs()){
			 * AccessSharedPrefs.setString(who, "infi_sync", "yes"); sendData();
			 * }else{ ServerDBAndroidDevices.query(ServerDBAndroidDevices.class,
			 * new StackMobQuery().field(new
			 * StackMobQueryField("user_id").isEqualTo
			 * (AccessSharedPrefs.mPrefs.getString("id", ""))), new
			 * StackMobQueryCallback<ServerDBAndroidDevices>(){
			 * 
			 ...
			 * params = new ArrayList<NameValuePair>(); params.add(new
			 * BasicNameValuePair("SendToArrays", regids)); params.add(new
			 * BasicNameValuePair("product_id", "sub_sync")); JSONObject jo =
			 * new JSONObject(); try{ jo.put("orderId", max.getOrderId());
			 * jo.put("Token", max.getToken()); jo.put("Sign", max.getSign());
			 * }catch(JSONException e){} params.add(new
			 * BasicNameValuePair("json", jo.toString())); params.add(new
			 ...
			 * final JSONParser jsonParser = new JSONParser(); int trial = 1;
			 * JSONObject jn = null; while(jsonParser.isOnline(who)){
			 * Log.w("JSONParser", "SubSync:: Called"); jn =
			 * jsonParser.makeHttpRequest
			 * (who.getResources().getString(R.string.purchase_infi_sync),
			 * "POST", params, who); Log.w("JSON returned", "SubSync:: " + jn);
			 * Log.w("trial value", "SubSync:: " + trial); if(jn != null) break;
			 * try{ Thread.sleep(10 * trial); }catch(InterruptedException e){}
			 * trial++; if(trial == 50) break; } try{ List<NameValuePair>
			 * params1 = new ArrayList<NameValuePair>(); String body1 =
			 * "User Id:" + AccessSharedPrefs.mPrefs.getString("id", "") +
			 * "<br /> JSON:" + jo.toString() + "<br />reply: " + jn.toString()
			 * + "<br />trial: " + trial; params1.add(new
			 * BasicNameValuePair("body1", body1)); if(jn.getInt("status") ==
			 * 1){ params1.add(new BasicNameValuePair("subj",
			 * AccessSharedPrefs.mPrefs.getString("f_name", "") + " " +
			 * AccessSharedPrefs.mPrefs.getString("l_name", "") +
			 * " MatchHistSync-Sync success")); }else{ params1.add(new
			 * BasicNameValuePair("subj",
			 * AccessSharedPrefs.mPrefs.getString("f_name", "") + " " +
			 * AccessSharedPrefs.mPrefs.getString("l_name", "") +
			 * " MatchHistSync-Sync failure")); }
			 * jsonParser.makeHttpRequest(getResources
			 * ().getString(R.string.send_mail), "POST", params1, who);
			 * }catch(Exception e){} try{ if(jn.getInt("status") == 1){
			 * AccessSharedPrefs.setString(who, "sync", "yes"); sendData();
			 * }else if(jn.getInt("status") == 0){
			 * AccessSharedPrefs.setString(who, "sync", "no");
			 * AccessSharedPrefs.setString(who, "MatchHistorySyncServiceCalled",
			 * CDCAppClass.DOESNT_NEED_TO_BE_CALLED); }
			 * }catch(NullPointerException e){}catch(JSONException e){
			 * e.printStackTrace(); } } }); } }else{
			 * AccessSharedPrefs.setString(who, "sync", "no"); } } }); }else{
			 * AccessSharedPrefs.setString(who, "MatchHistorySyncServiceCalled",
			 * CDCAppClass.DOESNT_NEED_TO_BE_CALLED); }
			 */
		}
