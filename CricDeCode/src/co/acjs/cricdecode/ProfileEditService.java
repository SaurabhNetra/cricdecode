...
public class ProfileEditService extends IntentService{
	...
	protected void onHandleIntent(Intent intent){
		...
		if(AccessSharedPrefs.mPrefs.getString("ProfileEditServiceCalled", CDCAppClass.DOESNT_NEED_TO_BE_CALLED).equals(CDCAppClass.NEEDS_TO_BE_CALLED)){
			ServerDBUserTable.query(ServerDBUserTable.class, new StackMobQuery().field(new StackMobQueryField("user_id").isEqualTo(AccessSharedPrefs.mPrefs.getString("id", ""))), new StackMobQueryCallback<ServerDBUserTable>(){
				@Override
				...
				public void success(List<ServerDBUserTable> arg0){
					if(arg0.size() != 0){
						...
						d.setId(arg0.get(0).getId());
						d.setBattingStyle(AccessSharedPrefs.mPrefs.getString("battingStyle", ""));
						d.setBowlingStyle(AccessSharedPrefs.mPrefs.getString("bowlingStyle", ""));
							...
							public void success(String arg0){
								...
								ServerDBAndroidDevices.query(ServerDBAndroidDevices.class, new StackMobQuery().field(new StackMobQueryField("user_id").isEqualTo(AccessSharedPrefs.mPrefs.getString("id", ""))), new StackMobQueryCallback<ServerDBAndroidDevices>(){
									...
									public void success(List<ServerDBAndroidDevices> arg0){
										...
										try{
											msg.put("gcmid", 1);
											msg.put("nickname", AccessSharedPrefs.mPrefs.getString("nickname", ""));
											msg.put("role", AccessSharedPrefs.mPrefs.getString("role", ""));
											msg.put("bowlingStyle", AccessSharedPrefs.mPrefs.getString("bowlingStyle", ""));
											msg.put("battingStyle", AccessSharedPrefs.mPrefs.getString("battingStyle", ""));
											Log.w("ProfileEditService: ", "" + msg.toString());
											final JSONParser jsonParser = new JSONParser();
											List<NameValuePair> params = new ArrayList<NameValuePair>();
											params.add(new BasicNameValuePair("SendToArrays", regids));
											params.add(new BasicNameValuePair("MsgToSend", msg.toString()));
											params.add(new BasicNameValuePair("uid", AccessSharedPrefs.mPrefs.getString("id", "")));
											Log.w("Sending User Data...", "ProfileEditService:" + jsonParser.isOnline(who));
											int trial = 1;
											JSONObject jn = null;
											...
											try{
												if(jn.getInt("status") == 1) AccessSharedPrefs.setString(who, "ProfileEditServiceCalled", CDCAppClass.DOESNT_NEED_TO_BE_CALLED);
