 ...
 */
package co.acjs.cricdecode.util;
 ...
 *
 * @author Bruno Oliveira (Veda) (Google)
 *
 */
public class IabHelper{
		...
		try{
			...
			if(mSubscriptionsSupported){
				try{
					...
					if(r != BILLING_RESPONSE_RESULT_OK){ throw new IabException(r, "Error refreshing inventory (querying owned subscriptions)."); }
					if(querySkuDetails){
						r = querySkuDetails(ITEM_TYPE_SUBS, inv, moreItemSkus);
						if(r != BILLING_RESPONSE_RESULT_OK){ throw new IabException(r, "Error refreshing inventory (querying prices of subscriptions)."); }
					}
				}catch(Exception e){}
			}
			return inv;
	...
	void consume(Purchase itemInfo) throws IabException{
		...
		if(!itemInfo.mItemType.equals(ITEM_TYPE_INAPP)){ throw new IabException(IABHELPER_INVALID_CONSUMPTION, "Items of type '" + itemInfo.mItemType + "' can't be consumed."); }
		try{
