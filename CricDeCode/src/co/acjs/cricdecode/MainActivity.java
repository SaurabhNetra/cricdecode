...
public class MainActivity extends SherlockFragmentActivity {
	...
	static final int NO_FRAGMENT = -1, SIGNIN_FRAGMENT = 9,
			...
			PURCHASE_FRAGMENT = 5, SUPPORT = 6, MATCH_CREATION_FRAGMENT = 7,
			PERFORMANCE_FRAGMENT_EDIT = 8, PERFORMANCE_FRAGMENT_VIEW = 9,
			PROFILE_EDIT = 10;
	static int root_fragment = CAREER_FRAGMENT;
	// Request Codes
	static final int PURCHASE_REMOVE_ADS = 398457, PURCHASE_INFI = 34809,
			PURCHASE_INFI_SYNC = 37867, PURCHASE_SYNC = 3561;
	// InAppBillingItems
	...
	protected void onCreate(Bundle savedInstanceState) {
									...
									if (result.isFailure()) {
										...
										AccessSharedPrefs
												.setString(main_context,
														"pur_infi_sync_title",
														"Unlimited Matches & Cloud Backup");
										AccessSharedPrefs
												.setString(
														main_context,
														"pur_infi_sync_price",
										...
										AccessSharedPrefs
												.setString(
														...
														inv.getSkuDetails(
																...
																.getDescription());
										AccessSharedPrefs.setString(
												main_context, "pur_sync_title",
												"Cloud Backup");
										AccessSharedPrefs.setString(
												main_context, "pur_sync_price",
												"Price");
										AccessSharedPrefs
												.setString(main_context,
														"pur_sync_descr",
				...
				if (result.isFailure()) {
					...
					if (purchase.getSku().equals(SKU_REMOVE_ADS)) {
						...
						AccessSharedPrefs.setString(main_context,
								...
								CDCAppClass.NEEDS_TO_BE_CALLED);
						AccessSharedPrefs.setString(main_context,
								"pur_sync_data", jo.toString());
						AccessSharedPrefs
								.setString(main_context, "sync", "yes");
						intent = new Intent(main_context,
		...
		if (savedInstanceState == null) {
			...
			case SUPPORT:
				spinner.setVisibility(View.GONE);
				SupportFragment.supportFragment = (SupportFragment) getSupportFragmentManager()
						.getFragment(savedInstanceState,
								"currentFragmentInstance");
				tx.setVisibility(View.VISIBLE);
			...
			public void run() {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						bundle.putString("color_bg", String.format(
								"#%06X",
								(0xFFFFFF & getResources().getColor(
										R.color.dark_red))));
						bundle.putString("color_bg_top", String.format(
								"#%06X",
								(0xFFFFFF & getResources().getColor(
										R.color.dark_red))));
						bundle.putString("color_border", String.format(
								"#%06X",
								(0xFFFFFF & getResources().getColor(
										R.color.dark_red))));
						bundle.putString("color_text", String.format(
								"#%06X",
								(0xFFFFFF & getResources().getColor(
										R.color.white))));
						bundle.putString("color_link", String.format(
								"#%06X",
								(0xFFFFFF & getResources().getColor(
										R.color.white))));
						bundle.putString("color_url", String.format(
								"#%06X",
								(0xFFFFFF & getResources().getColor(
										R.color.white))));
						SimpleDateFormat formatter = new SimpleDateFormat(
								"dd/MM/yyyy");
						String dateInString = AccessSharedPrefs.mPrefs
								.getString("dob", "");

						Calendar currentDate = Calendar.getInstance(); // Get
																		// the
																		// current
																		// date
						formatter = new SimpleDateFormat("dd/MM/yyyy"); // format
																		// it as
																		// per
																		// your
																		// requirement
						...
						try {
							...
							date2 = formatter.parse(dateNow);

						} catch (ParseException e) {
							e.printStackTrace();
						}
						Log.w("", "Yrs:" + date2.getTime() + " " + date1.getTime());
						
						long diffInYrs = date2.getTime() - date1
								.getTime();
						
						long div=31536000000l;
						long diff=diffInYrs/div;
						bundle.putString("Age", "" + diff);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		case SUPPORT:
			getSupportFragmentManager().putFragment(outState,
					"currentFragmentInstance", SupportFragment.supportFragment);
			break;
		case ANALYSIS_FRAGMENT:
			getSupportFragmentManager().putFragment(outState,
					"currentFragmentInstance",
	...
	protected void onStart() {
		...
		if (isAvailable == ConnectionResult.SUCCESS) {
			...
			new AlertDialog.Builder(this)
					...
					.setMessage("Unable to start app")
					.setNeutralButton("OK",
	...
	public void onClick(View view) {
		...
		case R.id.btn_fb_got_it:
			...
			((RelativeLayout) findViewById(R.id.fb_tip_rl))
					.setVisibility(View.GONE);
			break;
		case R.id.btn_add_to_hist:
			AccessSharedPrefs.setString(main_context, "hist_got_it", "yes");
			((RelativeLayout) findViewById(R.id.add_to_hist_rl))
					.setVisibility(View.GONE);
			break;
		case R.id.filter:
		...
		case R.id.add_match_to_career:
			...
			dialogText
					.setText("Are you sure you want to Add this Match to History? You will not be able to edit this match again!");
											...
											try {
												new AlertDialog.Builder(
														...
														.setTitle(
																"Not Applicable!")
														.setMessage(
																"This item is not applicable for you as you have already purchased a pack which contains this feature")
														.setNeutralButton(
																"OK",
																new DialogInterface.OnClickListener() {
																	public void onClick(
		...
		case R.id.pur_subscribe_sync:
						...
						public void run() {
							try {
								new AlertDialog.Builder(main_context)
										...
										.setMessage(
												"This feature is in the making. You can avail it very soon!")
										.setNeutralButton(
												"OK",
												new DialogInterface.OnClickListener() {
													public void onClick(
			...
			/*
			 * try{ writeToFile("onclick sync");
			 * mHelper.launchSubscriptionPurchaseFlow(this, SKU_SUB_SYNC,
			 * PURCHASE_SYNC, mPurchaseFinishedListener, getMD5());
			 * }catch(Exception e){ Toast.makeText(this,
			 * "Please retry in a few seconds.", Toast.LENGTH_SHORT).show(); }
			 */
			break;
		default:
			break;
		...
		try {
			if (session != null) {
				...
				String description = String
						.format("Batting: %s<center/> Bowling: %s<center/> Fielding: %s",
								bat, bowl, field);
				...
				params.putString("link", "cdc.acjs.co/sharedHighlights.php?fn="
						...
						+ team_a + "&tb=" + team_b + "&v=" + venue + "&bat="
						+ bat + "&bowl=" + bowl + "&field=" + field);
				params.putString("description", description);
				params.putString("picture",
						"https://dl.dropboxusercontent.com/u/15057367/Icons/CDC-icon.png");
