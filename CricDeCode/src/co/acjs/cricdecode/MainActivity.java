...
public class MainActivity extends SherlockFragmentActivity{
										...
										if(result.isFailure()){}else{
											...
											if(inventory.hasPurchase(SKU_SUB_INFI)){
												...
												if(p1.getDeveloperPayload().equals(getMD5())){
													...
													int c = r.nextInt(5);
													if(c == 0){
														AccessSharedPrefs.setString(MainActivity.main_context, "InfiChkServiceCalled", CDCAppClass.NEEDS_TO_BE_CALLED);
														main_context.startService(new Intent(main_context, InfiCheckService.class));
													}
			...
			public void run(){
				...
				runOnUiThread(new Runnable(){
					...
					public void run(){
						...
						bundle.putString("color_bg_top", String.format("#%06X", (0xFFFFFF & getResources().getColor(R.color.dark_red))));
						bundle.putString("color_border", String.format("#%06X", (0xFFFFFF & getResources().getColor(R.color.dark_red))));
						bundle.putString("color_text", String.format("#%06X", (0xFFFFFF & getResources().getColor(R.color.white))));
						bundle.putString("color_link", String.format("#%06X", (0xFFFFFF & getResources().getColor(R.color.white))));
						bundle.putString("color_url", String.format("#%06X", (0xFFFFFF & getResources().getColor(R.color.white))));
						SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");
						String dateInString = AccessSharedPrefs.mPrefs.getString("dob", "");
	...
	private void makeBarsTranslucent(Window window){
		...
		if(getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) actionBarHeight = TypedValue.complexToDimensionPixelSize(tv.data, getResources().getDisplayMetrics());
	...
	public void restart_services(){
		...
		boolean NotChkInfi = AccessSharedPrefs.mPrefs.getString("InfiChkServiceCalled", CDCAppClass.DOESNT_NEED_TO_BE_CALLED).equals(CDCAppClass.NEEDS_TO_BE_CALLED);
		if(NotSyncedMatchHistory | NotSyncedEditProfile | NotDeleted | NotAdRemoved | NotInfied | NotInfiSynced | NotSyncedGCMSync | NotSynced | NotChkInfi){
			...
			if(isConnected){
				...
				if(NotChkInfi & isConnected){
					Log.w("Starting PurchasedSyncService", "restart service");
					startService(new Intent(main_context, InfiCheckService.class));
				}
