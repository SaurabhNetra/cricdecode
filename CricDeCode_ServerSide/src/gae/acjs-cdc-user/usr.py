...
class usr_mig(webapp2.RequestHandler):
	...
	url = "http://api.stackmob.com/serverdbusertable"
        ...
        for data in datarr:
            obj_list = usr.query(usr.user_id == data['user_id']).fetch()
            if(len(obj_list) == 0):
                user_obj = usr()
                user_obj.batting_style = data['battingstyle']
                user_obj.bowling_style = data['bowlingstyle']
                user_obj.device_no = int(data['device_no'])
                try:
                    user_obj.dob = data['dob']
                except:
                    user_obj.dob = ""
                user_obj.fb_link = data['fb_link']
                user_obj.first_name = data['first_name']
                user_obj.os = int(data['hasandroid'])
                user_obj.last_name = data['last_name']
                user_obj.nick_name = data['nick_name']
...
class infisync_mig(webapp2.RequestHandler):
    def post(self):
        ...
        datarr = json.loads(res)
        self.response.write(len(datarr))        
        url = "http://acjs-cdc-infisync.appspot.com/insert"
        for data in datarr:
            values = {}
            values['autorenewing'] = data['autorenewing']
            values['initiation_ts_msec'] = data['initiation_ts_msec']
...
class infi_mig(webapp2.RequestHandler):
    def post(self):
        ...
        datarr = json.loads(res)
        self.response.write(len(datarr))        
        url = "http://acjs-cdc-infi.appspot.com/insert"
        for data in datarr:
            values = {}
            values['autorenewing'] = data['autorenewing']
            values['initiation_ts_msec'] = data['initiation_ts_msec']
